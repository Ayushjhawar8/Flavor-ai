...
export async function POST(req) {
  try {
    ...
    const isVegetarianUser = dietaryRestrictions
      .map((r) => r.toLowerCase())
      .includes("vegetarian");
    ...
    const vegetarianMeals = [
      {
        ...
        protein: Math.round((targetCalories * 0.25 * 0.2) / 4),
        carbs: Math.round((targetCalories * 0.25 * 0.6) / 4),
        fat: Math.round((targetCalories * 0.25 * 0.2) / 9),
        fiber: 8,
        sodium: 150,
        ingredients: [
      ...
      },
      {
        ...
        carbs: Math.round((targetCalories * 0.35 * 0.5) / 4),
      ...
      },
      {
        ...
        protein: Math.round((targetCalories * 0.3 * 0.2) / 4),
        carbs: Math.round((targetCalories * 0.3 * 0.65) / 4),
        fat: Math.round((targetCalories * 0.3 * 0.15) / 9),
        fiber: 8,
        sodium: 250,
        ingredients: [
      ...
      },
      {
        ...
        protein: Math.round((targetCalories * 0.1 * 0.2) / 4),
        carbs: Math.round((targetCalories * 0.1 * 0.6) / 4),
        fat: Math.round((targetCalories * 0.1 * 0.2) / 9),
        fiber: 3,
        sodium: 50,
        ingredients: [
    ...
    const nonVegMeals = [
      ...
      },
      {
        ...
        carbs: Math.round((targetCalories * 0.35 * 0.3) / 4),
      ...
      },
      {
        ...
        type: "dinner",
        calories: Math.round(targetCalories * 0.3),
        protein: Math.round((targetCalories * 0.3 * 0.4) / 4),
        carbs: Math.round((targetCalories * 0.3 * 0.35) / 4),
        fat: Math.round((targetCalories * 0.3 * 0.25) / 9),
        fiber: 6,
        sodium: 300,
        ingredients: [
      ...
      },
      {
        ...
        type: "snack",
        calories: Math.round(targetCalories * 0.1),
        protein: Math.round((targetCalories * 0.1 * 0.3) / 4),
        carbs: Math.round((targetCalories * 0.1 * 0.4) / 4),
        fat: Math.round((targetCalories * 0.1 * 0.3) / 9),
        fiber: 3,
        sodium: 50,
        ingredients: [
          { name: "Greek yogurt", amount: "1/2 cup" },
    ...
    const mockDietPlan = {
      ...
      dietPlan: {
        ...
        healthNotes: [
          bloodSugar !== "normal"
            ? "Monitor blood sugar levels and consider smaller, more frequent meals"
            : "Maintain stable blood sugar with balanced meals",
          bloodPressure !== "normal"
            ? "Keep sodium intake moderate and focus on potassium-rich foods"
            : "Continue heart-healthy eating patterns",
          goal === "bulk"
            ? "Ensure adequate protein intake for muscle growth"
            : goal === "cut"
              ? "Maintain protein to preserve muscle during weight loss"
