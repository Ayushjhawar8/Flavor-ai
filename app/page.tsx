...
export default function Page() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main
        className={`flex-grow flex flex-col items-center justify-center w-full bg-base-100 transition-all duration-300 relative z-10 ${
          !showResults ? "opacity-100" : "opacity-80 blur-sm"
        }`}
      >
        <section className="w-full min-h-screen bg-base-100 flex items-center justify-center relative z-10">
          {/* Floating background shapes */}

          <div
            className={`absolute top-0 left-0 w-72 h-72 rounded-full filter blur-3xl animate-blob1 mix-blend-multiply ${
              currentTheme === "dark" ? "bg-purple-700/30" : "bg-purple-400/30"
            }`}
          ></div>

          <div
            className={`absolute bottom-0 right-0 w-96 h-96 rounded-full filter blur-3xl animate-blob2 mix-blend-multiply ${
              currentTheme === "dark" ? "bg-pink-700/30" : "bg-pink-400/30"
            }`}
          ></div>

          <div
            className={`absolute top-1/2 left-1/3 w-60 h-60 rounded-full filter blur-2xl animate-blob3 mix-blend-multiply ${
          ...
          <div
            data-aos="fade-up"
            className="relative max-w-5xl mx-auto px-6 py-16 flex flex-col items-center text-center space-y-10 mt-[75px]"
          >
            {/* Heading */}
            <h1
            ...
            <div className="flex flex-wrap items-center justify-center gap-5">
              {[
                ...
                },
                {
                  href: "/random",
                  icon: (
                    <GiDiceSixFacesFour className="text-2xl" color="#FF6347" />
                  ),
             ...
             hover:scale-105 hover:shadow-xl transition-all duration-300"
              ...
              >
                {showCategories ? (
                  <>
                    <FaFolderOpen className="text-2xl" color="#F39C12" /> Hide
                    Categories
                  </>
                ) : (
                  <>
                    <FaFolder className="text-2xl" color="#F39C12" /> Show
                    Categories
                  </>
                )}
              </button>
