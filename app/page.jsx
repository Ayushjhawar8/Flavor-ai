...
export default function Page() {
  ...
  // Filter the categories based on filter selection
  const filteredCategories = categories.filter((category) => {
    if (filter === "All") return true;

    const catName = category.strCategory.toLowerCase();

    if (filter === "Vegetarian") {
      return catName.includes("vegetarian") || catName.includes("veg");
    }
    if (filter === "Non-Vegetarian") {
      // Assuming categories not including veg/vegetarian terms are non-veg
      return !catName.includes("vegetarian") && !catName.includes("veg");
    }
    ...
    <>
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-4">
          <Link href="#" id="main" className="btn btn-ghost text-2xl font-bold">
            Flavor AI
          </Link>
          <a
      ...
      >
        <section className="w-full h-screen bg-base-100 flex items-center justify-center">
          <div className="max-w-4xl mx-auto px-6 flex flex-col items-center text-center space-y-6 md:space-y-8">
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-4">
              ...
              {/* Show Categories + Filter Icon (included inside button for simplicity) */}
              <button
            ...
            {filterOptions.map((option) => (
              <button
                ...
                className={`btn btn-sm ${
                  filter === option ? "btn-primary" : "btn-outline"
                }`}
        ...
        {showCategories && (
          <section
            ...
            className="categories-section flex flex-col items-center justify-center p-5 md:p-10 w-full bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/30 dark:to-amber-800/20 rounded-lg shadow-lg"
          >
        ...
        <footer className="footer rounded-md mt-10 p-10 bg-base-200 text-base-content footer-center">
          <div className="flex flex-col md:flex-row justify-between items-center w-full max-w-4xl mx-auto space-y-4 md:space-y-0">
            ...
            <div className="text-sm text-center md:text-right">
              <p>&copy; {new Date().getFullYear()} Flavor AI. All Rights Reserved.</p>
            </div>
          </div>
        </footer>
      </div>
