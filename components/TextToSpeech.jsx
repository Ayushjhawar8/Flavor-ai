  ...
  useEffect(() => {
    ...
    if (isPlaying && !isPaused) {
      interval = setInterval(() => {
        const totalDuration = utteranceQueue.current.reduce(
          (acc, utt) => acc + utt.text.length / 10,
          0,
        ); // Estimate duration
        const elapsedTime = utteranceQueue.current
          .slice(0, currentUtteranceIndex.current)
          .reduce((acc, utt) => acc + utt.text.length / 10, 0);
