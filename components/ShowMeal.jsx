...
function IngredientsTable({ mealData })
{
  ...
  return (<div className="overflow-x-auto mt-2"><table className="table w-full"><thead><tr className="text-left"><th className="p-2 w-1/3 text-sm font-semibold text-base-content/70">Quantity</th><th className="p-2 text-sm font-semibold text-base-content/70">Ingredient</th></tr></thead><tbody>{ingredients.map((ing, i) => <tr key={i} className="border-t border-base-300 hover:bg-base-200"><td className="p-2 font-medium text-primary">{ing.measure}</td><td className="p-2 text-base-content">{ing.name}</td></tr>)}</tbody></table></div>);
}
...
function ShowMeal({ URL })
{
  ...
  if (!mealData)
  {
    return <div className="min-h-screen flex justify-center items-center p-4"><div className="max-w-4xl w-full p-12 bg-base-100 rounded-xl shadow-md"><div className="animate-pulse"><div className="h-10 bg-base-300 rounded-md w-3/4 mx-auto mb-4"></div><div className="h-6 bg-base-300 rounded-md w-1/4 mx-auto mb-10"></div><div className="flex flex-col md:flex-row gap-12"><div className="md:w-1/2"><div className="h-80 bg-base-300 rounded-lg"></div></div><div className="md:w-1/2 space-y-4"><div className="h-8 bg-base-300 rounded-md w-1/2"></div><div className="h-5 bg-base-300 rounded-md"></div><div className="h-5 bg-base-300 rounded-md"></div><div className="h-5 bg-base-300 rounded-md"></div><div className="h-5 bg-base-300 rounded-md"></div></div></div></div></div></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-base-100 flex flex-col">
      ...
      <div className="flex-1 py-10 px-4 flex justify-center items-start">
        <BackButton />
        <div className="relative max-w-4xl w-full bg-base-100 shadow-xl rounded-xl">
          <div className="p-6 md:p-12">
            <header className="text-center mb-8"><h1 className="text-3xl md:text-5xl font-bold text-base-content">{mealData.strMeal}</h1><p className="text-lg text-base-content/70 mt-2">{mealData.strArea} Cuisine</p></header>
            <div className="flex flex-col md:flex-row gap-8 md:gap-12 mb-12"><div className="md:w-1/2"><img src={mealData.strMealThumb} alt={mealData.strMeal} className="w-full h-auto rounded-lg shadow-md mb-4" /><div className="flex items-center gap-4"><span className="badge badge-lg badge-accent">{mealData.strCategory}</span>{mealData.strYoutube && (<Link href={mealData.strYoutube} target="_blank" rel="noopener noreferrer" className="btn btn-error btn-sm gap-2"><YoutubeIcon /> Watch</Link>)}</div></div><div className="md:w-1/2"><h2 className="text-2xl font-bold mb-2 flex items-center text-base-content"><PlusIcon /><span className="ml-2">Ingredients</span></h2><IngredientsTable mealData={mealData} /></div></div>
