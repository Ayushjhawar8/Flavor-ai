...
function testGraphInitialization() {
  ...
  console.log(
    `✅ Graph initialized with ${stats.nodeCount} nodes and ${stats.edgeCount} edges`,
  );
  return true;
}
...
function testSimilarityCalculations() {
  ...
  testIngredients.forEach((ingredient) => {
    const similarity = ingredientGraph.calculateSimilarity(
      ingredient,
      ingredient,
    );
    if (similarity !== 1.0) {
      console.error(
        `❌ Self-similarity for ${ingredient} should be 1.0, got ${similarity}`,
      );
      allPassed = false;
    }
  });
  ...
  testPairs.forEach(([ing1, ing2]) => {
    ...
    if (similarity < 0.3) {
      console.error(
        `❌ Similarity between ${ing1} and ${ing2} is too low: ${similarity}`,
      );
      allPassed = false;
    } else {
      console.log(
        `✅ ${ing1} ↔ ${ing2}: ${Math.round(similarity * 100)}% similarity`,
      );
    }
  });
...
function testComplementarySuggestions() {
  ...
  testIngredients.forEach((ingredient) => {
    const complements = ingredientGraph.findComplementaryIngredients(
      ingredient,
      3,
    );
    ...
    if (complements.length === 0) {
      ...
      console.log(
        `✅ ${ingredient} → ${complements.map((c) => `${c.ingredient} (${Math.round(c.score * 100)}%)`).join(", ")}`,
      );
    }
  });
...
function testSubstituteSuggestions() {
  ...
  testSubstitutes.forEach((ingredient) => {
    const substitutes = ingredientGraph.findSubstituteIngredients(
      ingredient,
      3,
    );
    ...
    if (substitutes.length === 0) {
      console.warn(
        `⚠️ No substitutes found for ${ingredient} (this might be expected)`,
      );
    } else {
      console.log(
        `✅ ${ingredient} → ${substitutes.map((s) => `${s.ingredient} (${Math.round(s.score * 100)}%)`).join(", ")}`,
      );
    }
  });
...
function testPairingAnalysis() {
  ...
  testIngredientLists.forEach((ingredients, index) => {
    ...
    if (!pairing.complementary && !pairing.substitutes) {
      console.error(
        `❌ No pairing suggestions for ingredients: ${ingredients.join(", ")}`,
      );
      allPassed = false;
    } else {
      ...
      if (pairing.complementary.length > 0) {
        console.log(
          `   Complementary: ${pairing.complementary.map((c) => c.ingredient).join(", ")}`,
        );
      }
      if (pairing.substitutes.length > 0) {
        console.log(
          `   Substitutes: ${pairing.substitutes.map((s) => s.ingredient).join(", ")}`,
        );
      }
    }
  });
...
function testEdgeCases() {
  ...
  const nonExistentSimilarity = ingredientGraph.calculateSimilarity(
    "nonexistent1",
    "nonexistent2",
  );
  if (nonExistentSimilarity !== 0.0) {
    console.error(
      `❌ Non-existent ingredients should have 0 similarity, got ${nonExistentSimilarity}`,
    );
    allPassed = false;
  }
  ...
  if (
    emptyPairing.complementary.length !== 0 ||
    emptyPairing.substitutes.length !== 0
  ) {
  ...
  const singlePairing = ingredientGraph.generatePairingSuggestions(
    ["tomato"],
    3,
  );
...
function runAllTests() {
  ...
  if (passedTests === totalTests) {
    console.log(
      "🎉 All tests passed! The graph-based ingredient similarity model is working correctly.",
    );
  } else {
