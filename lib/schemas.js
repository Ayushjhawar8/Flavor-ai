 ...
 * Used for validating comment submissions and responses
 */
const commentSchema = z.object({
    id: z.string().describe('Unique identifier for the comment.'),
    recipeId: z.string().describe('ID of the recipe this comment belongs to.'),
    nickname: z.string().min(1).max(50).default('Anonymous').describe('Commenter nickname (optional).'),
    comment: z.string().min(10).max(500).describe('The comment content.'),
    timestamp: z.string().describe('ISO timestamp when comment was created.'),
    ipAddress: z.string().optional().describe('IP address for spam prevention.'),
}).describe('A comment on a recipe.');

/**
 * Schema for comment submission (what users send)
 */
export const commentSubmissionSchema = z.object({
    ...
    captchaAnswer: z.number().describe('Answer to the math captcha.'),
}).describe('Data required to submit a new comment.');

/**
 * Schema for comment responses (what API returns)
 */
export const commentsResponseSchema = z.object({
    comments: z.array(commentSchema).describe('Array of comments for a recipe.'),
    total: z.number().describe('Total number of comments.'),
}).describe('Response containing recipe comments.');
