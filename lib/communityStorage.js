...
export const addToCollection = (
  ...
  if (allCollections[userId]) {
    const collection = allCollections[userId].find(
      (c) => c.id === collectionId,
    );
    if (collection) {
...
export const getRecipeNotes = (recipeId) => {
  const stored = localStorage.getItem("recipe_notes");
  const allNotes = stored ? JSON.parse(stored) : {};
  return allNotes[recipeId] || "";
};

export const saveRecipeNotes = (recipeId, notes) => {
  const stored = localStorage.getItem("recipe_notes");
  const allNotes = stored ? JSON.parse(stored) : {};
  allNotes[recipeId] = notes;
  localStorage.setItem("recipe_notes", JSON.stringify(allNotes));
};

export const deleteRecipeNotes = (recipeId) => {
  const stored = localStorage.getItem("recipe_notes");
  const allNotes = stored ? JSON.parse(stored) : {};
  delete allNotes[recipeId];
  localStorage.setItem("recipe_notes", JSON.stringify(allNotes));
};
