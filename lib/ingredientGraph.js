...
class IngredientGraph {
  ...
  findComplementaryIngredients(ingredient, limit = 5) {
    ...
    for (const node of this.nodes) {
      if (node !== ingredient) {
        ...
        if (similarity > 0.3) {
          // Threshold for meaningful complementarity
          complements.push({ ingredient: node, score: similarity });
        }
      }
    }

    return complements.sort((a, b) => b.score - a.score).slice(0, limit);
  }
  ...
  findSubstituteIngredients(ingredient, limit = 3) {
    ...
    for (const node of this.nodes) {
      if (node !== ingredient) {
        const similarity = this.calculateSimilarity(ingredient, node);
        if (similarity > 0.7) {
          // High threshold for substitutability
          substitutes.push({ ingredient: node, score: similarity });
        }
      }
    }

    return substitutes.sort((a, b) => b.score - a.score).slice(0, limit);
  }
  ...
  generateReasoning(baseIngredients, complementary, substitutes) {
    ...
    if (complementary.length > 0) {
      ...
      reasons.push(
        `${topComplement.ingredient} pairs well with your ingredients due to complementary flavor profiles.`,
      );
    }
    ...
    if (substitutes.length > 0) {
      ...
      reasons.push(
        `${topSubstitute.ingredient} can be used as a substitute if you're missing any ingredients.`,
      );
    }
